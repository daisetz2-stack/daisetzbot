...
1. **Develop the job description with the user.** Ask clarifying questions if anything is ambiguous — especially if the task involves changes to daisetz's own codebase.
...
- Modifying the daisetz codebase itself
...
Below are technical details on how daisetz is built.
- Use these to help generate a solid plan when creating tasks or jobs that modify daisetz codebase
