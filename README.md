...
**Secure by default** — Other frameworks hand credentials to the LLM and hope for the best. daisetz is different: the AI literally cannot access your secrets, even if it tries. Secrets are filtered at the process level before the agent's shell even starts.
...
**Free compute, built in** — Every GitHub account comes with free cloud computing time. daisetz uses that to run your agent. One task or a hundred in parallel — the compute is already included.
